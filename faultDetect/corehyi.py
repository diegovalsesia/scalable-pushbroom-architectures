...
"""
this module contains the core functionality for fault injections, including:
this is modified from the original pytorchfi code: https://github.com/pytorchfi/pytorchfi/blob/master/pytorchfi/core.py 
we thank the original authors for their work and contribution to the field of fault injection in deep learning.
"""
...
class fault_injection:
    def __init__(self, model, batch_size, input_shape=None, layer_types=None, **kwargs):
        ...
        device = 'cuda:1' if self.use_cuda else None
    ...
    def layer_weights_size_random(self):
        """
        Returns a list with size of the weights of each layer in the model.
        """
        layer_weights_size = []
        for layer, _dim in enumerate(self.output_size):
            layer_weights_size.append(torch.rand(self.weights_size[layer],dtype=torch.float64))
        return layer_weights_size
    def fi_reset(self):
        ...
        for layer in self.corrupted_model.modules():
            if isinstance(layer, tuple(self.get_inj_layer_types())):
                ...
                for inj in inj_list:
                    ...
                    revised_corrupt_idx = corrupt_idx[: len(layer_shape)]
                    corrupt_idx = tuple(
                        [
                            min(max(0, revised_corrupt_idx[i]), layer_shape[i] - 1)
    ...
    def print_pytorchfi_layer_summary(self):
        ...
        tot_params = 0
        for layer, _dim in enumerate(self.output_size):
            tot_params += self.weights_size[layer].numel()
        summary_str += "Total parameters: " + str(tot_params) + "\n\n"
        for layer, _dim in enumerate(self.output_size):
            line_new = "{:>5}  {:>15}  {:>10} {:>20} {:>20}".format(
                layer,
