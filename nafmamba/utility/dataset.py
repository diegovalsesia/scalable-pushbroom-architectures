...
try: 
    ...
    try:
        ...
        from nafmamba.utility.util import *
...
class AddNiidNoiseBlind(object):
    """add blind gaussian noise to the given numpy array (B,H,W)"""
    def __pos(self, n):
        i = 0
        while True:
    ...
    def __call__(self, img):
        noise = np.random.randn(*img.shape)
        sigma_ratio = self.sigmas[next(self.pos)]
        for z in range(img.shape[0]):
            noise[z,:,:]=np.random.randn(*img[z,:,:].shape)* random.random()*sigma_ratio
        # print(img.sum(), noise.sum())
        return img + noise
...
if __name__ == '__main__':
    """Mat dataset test"""
    # dataset = MatDataFromFolder('your_path')
    # mat = dataset[0]    
