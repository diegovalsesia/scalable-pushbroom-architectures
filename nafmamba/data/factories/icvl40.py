...
from nafmamba.data.splits import (
    icvl40_train,
    icvl40_val,
    icvl40_test,
    icvl40_crops,
...
class ICVL40(DatasetFactory):
    ...
    def __init__(self, **kwargs):
        ...
        self.f_train = icvl40_train
        self.f_val = icvl40_val
        self.f_test = icvl40_test
    ...
    def download(cls, path_data):
        ...
        icvl40_all = icvl40_train + icvl40_val + icvl40_test
        icvl40_all = [f"{fn}.mat" for fn in icvl40_all]
        for i, filename in enumerate(icvl40_all):
            target = os.path.join(path_raw, filename)
            url = os.path.join(BASE_URL, filename)
            logger.info(
                f"Checking image ({i + 1}/{len(icvl40_all)}) : {filename}"
            )
    ...
    def preprocess(self):
        ...
        icvl40_all = list(set(self.f_train + self.f_test + self.f_val))

        for i, fn in enumerate(icvl40_all):
            ...
            logger.info(
                f"Saved normalized image {i + 1}/{len(icvl40_all)} "
                f"to {path_out}"
            )
        touch(path_complete)
