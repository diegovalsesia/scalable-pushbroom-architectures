...
from nafmamba.data.datasets import (
    LMDBDataset,
    NoisyTransformDataset,
    PathsDataset,
    MatImageDataset
)
from nafmamba.utils.dataloaders_hsi_test import Dataset
from nafmamba.data.patches_utils import generate_patches
from nafmamba.data.transforms import RandomRot90, RandomSpectralInversion
logger = logging.getLogger(__name__)
logger.setLevel(logging.DEBUG)


class DatasetFactory(ABC):
    def __init__(self, noise, path_data, seed, split, bands=None, ssl=False,val_noisy_path=None, val_gt_path=None):
        self.noise = noise
        ...
        self.bands = bands
        self.split = split
        if val_gt_path is not None:
            self.val_gt_path = to_absolute_path(val_gt_path)
            print('val_gt_path', self.val_gt_path)
        if val_noisy_path is not None:
            self.val_noisy_path = to_absolute_path(val_noisy_path)
            print('val_noisy_path', self.val_noisy_path)
    ...
    def val(self):
        if self.val_noisy_path is not None and self.val_gt_path is not None:
            print('getting val dataset from mat files')
            return MatImageDataset(noisy_path=self.val_noisy_path, gt_path=self.val_gt_path)
        else:
