...
from nafmamba.models.layers.ssrt_modules import BasicLayer
...
class RecurrentWindowAttention(nn.Module):
    ...
    def forward(self, input_x, state_x, mask=None):
        ...
        cross_attn_s = self.attn_drop(cross_attn_s) # different drop!!!
...
class BlockRecurrentSwinIRBlock(nn.Module):
    ...
    def forward(self, x, state, x_size,x_size_next):
        ...
        state = self.norm1_state(state.to('cuda'))
...
class BasicLayer_bidir_01(nn.Module):
    ...
    def forward(self, x_list, x_size,x_size_next, state=None):
        ...
        for x_ in range(len(x_list)):
            x_list_[x_] = x_list_[x_]
            x_list_2[x_] = x_list_2[x_]
        ...
        for blk_id in range(len(self.blocks)): 
            for i in range(len(x_list)):
                if self.use_checkpoint:
                    tmp, state = checkpoint.checkpoint(self.blocks[blk_id], x_list_[i].to('cuda'), state, x_size,x_size_next)
                    x_list_[i] = tmp.to('cpu')
                    torch.cuda.empty_cache()
                else:
                    tmp, state = self.blocks[blk_id](x_list_[i].to('cuda'), state, x_size,x_size_next)
                    x_list_[i] = tmp.to('cpu')
                    torch.cuda.empty_cache()
            for i in range(len(x_list)):
                if self.use_checkpoint:
                    tmp, state_back = checkpoint.checkpoint(self.blocks_back[blk_id], x_list_2[i].to('cuda'), state_back, x_size,x_size_next)
                    x_list_2[i]=tmp.to('cpu')
                    torch.cuda.empty_cache()
                else:
                    tmp, state_back = self.blocks_back[blk_id](x_list_2[-i+1].to('cuda'), state_back, x_size,x_size_next)
                    x_list_2[-i+1]=tmp.to('cpu')
                    torch.cuda.empty_cache()
        x_list_res = add_list(x_list_, x_list_2)
        if self.downsample_flag == PatchUnmerging:
            for i in range(len(x_list_res)):
                x_list_[i] = self.downsample(x_list_res[i].to('cuda'), x_size).to('cpu')
        elif self.downsample_flag == PatchMerging:
            for i in range(len(x_list_res)):
                x_list_[i] = self.downsample(x_list_res[i].to('cuda')).to('cpu')
        else:
...
class BRRSTB(nn.Module):
    ...
    def forward(self, x_list, x_size):
        ...
        x_list__ = []
        for i in range(len(x_list)):
            x_list__.append(self.patch_unembed_next(x_list_[i], x_size_next).to('cuda'))
        ...
        if self.downsample_flag == PatchUnmerging:
            for i in range(len(x_list)):
                x_list[i] = self.downsample(x_list[i].to('cuda'), (x_size_next[0]//2,x_size_next[1]//2))
      
        elif self.downsample_flag == PatchMerging:
            for i in range(len(x_list)):
                x_list[i] = self.downsample(x_list[i].to('cuda'))
...
class ssrt(nn.Module):
    ...
    def forward_features(self, x_list):
        ...
        #     scipy.io.savemat('your_path'+str(oo)+'.mat', {'data':temp})
            
        for x__ in range(len(x_list_)):
            x_list_[x__] = x_list_[x__].to('cpu')
        for x__ in range(len(res_group)):
            for y__ in range(len(res_group[x__])):
                res_group[x__][y__] = res_group[x__][y__].to('cpu')
        ...
        x_list__ = []
        for i in range(len(x_list)):
            x_list__.append(self.norm(x_list_[i].to('cuda')))  # B L C
