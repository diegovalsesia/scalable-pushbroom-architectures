...
class CausalConv1d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, dilation=1):
        ...
        self.conv = nn.Conv1d(in_channels, out_channels, kernel_size, stride=stride,
                              padding=self.padding, groups=out_channels, dilation=dilation)
...
class CausalConv1d2(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, dilation=1):
        super(CausalConv1d2, self).__init__()
        self.padding = (kernel_size - 1) * dilation
        self.conv = nn.Conv1d(in_channels, out_channels, kernel_size, stride=stride,
                              padding=self.padding, groups=1, dilation=dilation)
...
#        self.batch_norm1 = nn.BatchNorm2d(c)
...
###        if eval is True:
####            print("eval is True")
###            x = rearrange(x, "(B H) F W -> B H F W", H=1)
###            x = rearrange(x, "B H F W -> B W H F")
###            x = rearrange(x, "B W H F -> (B W) H F")
###            x = self.SSM(x, inference_params=inference_params, index_initialize_states=index_initialize_states)
###        else:
####            print("eval is False")
###            x = rearrange(x, "(B H) F W -> B H F W", H=32)
###            x = rearrange(x, "B H F W -> B W H F")
###            x = rearrange(x, "B W H F -> (B W) H F")
###            x = self.SSM(x)
###            
###        x = rearrange(x, "(B W) H F -> B W H F", W=32)
###        x = rearrange(x, "B W H F -> B H F W")
###        x = rearrange(x, "B H F W -> (B H) F W")
        x = x * self.sca(x)
        x = self.conv3(x)
#        x = self.dropout1(x)
        y = inp + x * self.beta
...
#        )
    ...
    def forward(self, hidden_states, height= 32, inference_params=None, index_initialize_states=0):
        """
        hidden_states: (B, L, D)
        Returns: same shape as hidden_states
        """
...
#            out = rearrange(out, "(B W C) H F -> B W C H F", W=width, B=batch)
#            out = rearrange(out, " B W C H F -> B H F W C")
#            out = rearrange(out, "B H F W C -> (B H) F W C")
            return out
...
#        out = rearrange(y, "(B W C) H F -> B W C H F", W=width, B=batch)
#        out = rearrange(out, " B W C H F -> B H F W C")        
#        out = rearrange(out, "B H F W C -> (B H) F W C")        
        return y
...
#        self.final_channel_attention = ChannelGate(self.num_of_features, 202)
    ...
    def get_line_from_cache(self, inference_params, initialize_states=0, batch_size=1, height=1,  channels = 202, width = 32):
            if initialize_states == 0:
...
###        if inference is False:
        ...
        feature_out = rearrange(q_z, "(B H) F W -> B F H W", H=height)
