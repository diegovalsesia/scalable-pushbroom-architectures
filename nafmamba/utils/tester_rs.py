...
from nafmamba.utility import *
from . import dataloaders_hsi_test
from .channel_utils import build_channel_slices
...
from nafmamba.models.metrics import (
    mergas,
    mfsim,
    mpsnr,
    msam,
...
def norm(input):
    ...
    return output
    # mi = torch.amin(input, dim=(0, 1, 2), keepdim=True)
    # ma = torch.amax(input, dim=(0, 1, 2), keepdim=True)
    # return (input - mi) / (1e-6 + (ma - mi))
...
class Tester:
    ...
    def eval(self, model, datamodule):
        ...
        for i,(x,fname) in enumerate(tqdm(test['test'],disable=True)):
            ...
            with torch.no_grad():
                ...
                # kernel_size = (inputs.shape[-3],64,64)
                kernel_size = (inputs.shape[-3],divisible_by(inputs.shape[-2],32),divisible_by(inputs.shape[-1],32))
                stride = (31,176,176)
                self.device = 'cuda'
                ...
                for b in range(inputs.shape[0]):
                    if model.block_inference and model.block_inference.use_bi:
                        ...
                        for start, end in channel_slices:
                            chunk = inputs[b, :, start:end, :, :]
                            out_chunks.append(
                                model.forward_blocks(chunk.to(self.device))[0]
                            )

                        if remainder == 0:
                            out = torch.cat(out_chunks, dim=0).unsqueeze(0)
                        else:
                            out = torch.cat(
                                out_chunks[:-1] + [out_chunks[-1][-remainder:, :, :]],
                                dim=0,
                            ).unsqueeze(0)
                        if isinstance(out, tuple):
                            out = out[0]
                        outputs[b,:,:,:,:] = out.to('cpu')
                        ...
                        if isinstance(out, tuple):
                            out = out[0]
                        outputs[b,:,:,:,:] = out.to('cpu')
                        # outputs[b,:,:,:,:] = model.forward(inputs[b,:,:,:,:].to(self.device)).to('cpu')
                    torch.cuda.empty_cache()
                endtime=time.time()
                logger.debug(f"Block inference FLOPs sum: {flops_sum}")
                logger.debug(f"Block inference time: {endtime-start_time}")
                out = refold(outputs.to(inputs.device),data_shape=data_shape, kernel_size=kernel_size,stride=stride,device=self.device).unsqueeze(0).unsqueeze(0).float().to(self.device).squeeze(0)
    ...
    def _save_raw(self, x, out, img_id, y=None, crop=False, **kwargs):
        ...
        scipy.io.savemat(path_img,mdict={'nafmamba':out[0].cpu().numpy()})
        logger.debug(f"Image saved to {path_img!r}")
